...
public class MainActivity extends Activity implements OnClickListener,
    ...
    class MyAapter extends ArrayAdapter<User> {
        ...
        public MyAapter(ArrayList<User> users) {
            super(MainActivity.this, 0, users);
        }
            ...
            case R.id.login_btnLogin:
               ...
               // showLoginingDlg(); // 显示"正在登录"对话框,因为此Demo没有登录到web服务器,所以效果可能看不出.可以结合情况使用
                ...
                if (mIdString == null || mIdString.equals("")) { // 账号为空时
                    ...
                    try {
                        OkHttpUtils.post().
                                url("http://122.228.80.181:82/Index/Login/login.html")
                                .tag(MainActivity.this)
                                .addParams("username",mIdString).addParams("password",mPwdString)
                            ...
                            public void onResponse(String response, int id) {
                                ...
                                userModel.setUserId(resjson.getIntValue("user_id"));
                                userModel.setUserImg(resjson.getString("user_img"));
                                System.out.println("user_img:"+userModel.getUserImg());
                                if(null == userModel.getUserImg() || userModel.getUserImg().trim().length() == 0){
                                    Toast.makeText(MainActivity.this, "用户头像数据为空，无法登录", Toast.LENGTH_SHORT).show();
                                    return;
                                }
                                Intent intent = new Intent(MainActivity.this,OpenglActivity.class);
                                intent.putExtra("LoginUser",userModel);
